...
if (listaProductos) {
  ...
  for (let i = 0; i < limite; i++) {
    ...
    if (mostrarTodos) {
      li.innerHTML = `
        <a href="8detalle-productos.html?producto=${producto.id}">
      ...
      // index html
      li.innerHTML = `
        <a href="/Html/8detalle-productos.html?producto=${producto.id}">
          <img src="${producto.imagen}" alt="${producto.nombre}">
          <h3>${producto.nombre}</h3>
          Ver Detalles
        </a>
...
function agregarAlCarrito(id, cantidad = 1) {
  ...
  if (productoEnCarrito) {
    ...
    // Creamos una copia del producto y le asignamos cantidad
  ...
  alert(`${productoOriginal.nombre} x${cantidad} agregado al carrito`);
  window.location.href = "9carrito.html";
...
document.addEventListener('click', function (e) {
  if (e.target.classList.contains('agregar-carrito')) {
    ...
    const productoEl = button.closest('li');
    const cantidadInput = productoEl.querySelector('.cantidad-input');
